Feature: PWA 웹 푸쉬 / 알림 기능
  ...
  Scenario: 등록된 커스텀 응원 메시지를 삭제하는 경우
    Given 알림 권한 상태가 "granted" 이다
